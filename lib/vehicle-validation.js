...
export function validateVehicleCombination(carData, caravanData, licenseInfo) {
  ...
  // Validation 2: Check if trailer needs brakes (mandatory when > 750kg)
  const trailerNeedsBrakes = caravanWeight > 750;
  const trailerHasBrakes = hasBrakes(caravanData);
  
  if (trailerNeedsBrakes) {
    if (trailerHasBrakes) {
      validationSuccess.push(`✅ Aanhangwagen heeft verplichte remmen (gewicht: ${caravanWeight}kg > 750kg)`);
    } else {
      validationErrors.push(`❌ Aanhangwagen heeft geen remmen maar weegt meer dan 750kg (${caravanWeight}kg)`);
    }
  } else {
